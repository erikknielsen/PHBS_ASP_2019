...
def basket_price_mc(
    ...
    if( bsm ) :
        ...
        '''
        prices=(spot/np.exp(-texp*intr)*np.exp(-texp*divr))[:,None]*np.exp(-1/2*texp*vol[:,None]**2+np.sqrt(texp)*chol_m@znorm_m)
...
def basket_price_norm_analytic(
    ...
    PUT YOUR CODE BELOW
    '''
    price=normal_formula(strike, (spot/np.exp(-texp*intr)*np.exp(-texp*divr))@weights[:,None]*np.exp(-texp*intr), np.sqrt(weights@(vol*cor_m*vol[:,None])@weights[:,None]), texp, intr=intr, divr=divr, cp_sign=cp_sign)
    return price[1]
